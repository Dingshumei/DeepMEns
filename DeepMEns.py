...
print('x_data1',x_data1.shape)

x_data2=position_code(x_data,21)
print('posi',x_data2.shape)
...
bio_feature = pd.read_csv(r'D:\Wt_biofeature_add.csv',header=None)
bio_feature = np.array(bio_feature)

x_data_RNA_Struct = pd.read_csv(r'D:\Wt_RNAencoding.csv',header=None)                #简单的0-1编码
x_data_RNA_Struct = np.array(x_data_RNA_Struct)
x_data_RNA_Struct = np.expand_dims(x_data_RNA_Struct, axis=-1)  #增加维度
x_data_RNA_Struct = np.expand_dims(x_data_RNA_Struct, axis=1)
print('x_data_RNA',x_data_RNA_Struct.shape)

#5*21矩阵
model_input1 = np.concatenate((x_data1,x_data_RNA_Struct), axis=3)
print('model_input1',model_input1.shape)

x_train_posi,x_test_posi,x_train_bio,x_test_bio,x_train_oneRNA,x_test_oneRNA,x_train_one,x_test_one, x_train_shape,x_test_shape,y_train, y_test = train_test_split(x_data2,bio_feature,model_input1,x_data1, Wt_shape,y_data, test_size=0.15)
...
def attention_3d_block(inputs):
    # inputs.shape = (batch_size, time_steps, input_dim)    time_steps:序列长度，input_dim:嵌入维度
...
def DeepMEns_training(shape1, shape2,shape3,shape4):
    ...
    for train_index, valid_index in kf.split(x_train_one):
        ...
        model.save(r'D:\Wt_DeepMEns_5_{0}.h5'.format(N))
        model.summary()
        N = N + 1
f=DeepMEns_training(shape1=(21,5),shape2=(21,5),shape3=(21,),shape4=(17,))


def DeepMEns_testing(inputs):
    model0 = load_model(r'D:\Wt_DeepMEns_5_0.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model1 = load_model(r'D:\Wt_DeepMEns_5_1.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model2 = load_model(r'D:\Wt_DeepMEns_5_2.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model3 = load_model(r'D:\Wt_DeepMEns_5_3.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model4 = load_model(r'D:\Wt_DeepMEns_5_4.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    ...
    print('mse', mse)
    return y_pred
g=DeepMEns_testing([x_test_shape,x_test_oneRNA,x_test_posi,x_test_bio])
